    ...
    while (1)
    {
        ...
        for (int i = 0; i < static_cast<int>(rectbox.size()) - 1; i++)
        {
            ...
            if (m > 0 && m < 8 && n < 40000 && n > 2000)
            {
                ...
                line(img, vertices1[0], vertices2[2], Scalar(0, 0, 255), 1);
                line(img, vertices1[2], vertices2[0], Scalar(0, 0, 255), 1);
                double x1 = (vertices1[0].x + vertices2[2].x) / 2;
                double y1 = (vertices1[0].y + vertices2[2].y) / 2;
                circle(img, Point(x1, y1), 5, Scalar(0, 255, 255), -1);

                prebox.push_back(Point(x1, y1));
                if (vertices2[0].x >= vertices1[0].x)
                {
                    imgp.push_back(vertices1[2]);
                    imgp.push_back(vertices2[2]);
                    imgp.push_back(vertices2[0]);
        ...
        if (imgp.size() == 4)
        {
            ...
            double a1 = atan2(rotm.at(2, 3), rotm.at(3, 3));
            double c2 = sqrt(rotm.at(1, 1) * rotm.at(1, 1) + rotm.at(1, 2) * rotm.at(1, 2));
            double a2 = atan2(-rotm.at(1, 3), c2);
            double a3 = atan2(rotm.at(1, 2), rotm.at(1, 1));

            // double tx=tvecs.at<double>(0,0);
            // double ty=tvecs.at<double>(1,0);
            // double tz=tvecs.at<double>(2,0);
            // double angxoz=atan(ty/tx)*57.3;
            // double angxoy=atan(tz/ty)*57.3;
            // double angyoz=atan(tx/tz)*57.3;
            ...
            // cout<<angxoy<<"度  "<<angxoz<<"度  "<<angyoz<<"度"<<endl;
        }
